/**
* Name: NewLSystemModel_modified
* Based on the internal empty template. 
* Author: pchwx, modified by panprapacw
* Tags: 
*/
...
//	int shift_current_day <- 0 update: shift_cycle mod season_duration; 
	...
	/* 
	float energy1 <- 0.0 update: energy_map1[current_season];
	float energy2 <- 0.0 update: energy_map2[current_season];
	float energy3 <- 0.0 update: energy_map3[current_season];
	float energy4 <- 0.0 update: energy_map4[current_season];
	float energy5 <- 0.0 update: energy_map5[current_season];
	float energy6 <- 0.0 update: energy_map6[current_season];
	float energy7 <- 0.0 update: energy_map7[current_season];
	float energy8 <- 0.0 update: energy_map8[current_season];
	float energy9 <- 0.0 update: energy_map9[current_season];
	float energy10 <- 0.0 update: energy_map10[current_season];
	float energy11<- 0.0 update: energy_map11[current_season];
	float energy12<- 0.0 update: energy_map12[current_season];
	*/
	
	
	map<string, float> energy_map1 <- ["winter"::0.0, "spring"::0.3, "summer"::0.08, "autumn"::0];
	map<string, float> energy_map2 <- ["winter"::0.0, "spring"::1.0, "summer"::0.08, "autumn"::0.25];
	map<string, float> energy_map3 <- ["winter"::0.02, "spring"::1.0, "summer"::0.1, "autumn"::0.0];
	map<string, float> energy_map4 <- ["winter"::0.02, "spring"::1.5, "summer"::0.1, "autumn"::0.0];
	map<string, float> energy_map5 <- ["winter"::0.15, "spring"::1.5, "summer"::0.8, "autumn"::0.02];
	map<string, float> energy_map6 <- ["winter"::0.15, "spring"::2.0, "summer"::0.8, "autumn"::0.02];
	map<string, float> energy_map7 <- ["winter"::0.02, "spring"::2.0, "summer"::0.08, "autumn"::0.15];
	map<string, float> energy_map8 <- ["winter"::0.02, "spring"::1.5, "summer"::0.08, "autumn"::0.15];
	map<string, float> energy_map9 <- ["winter"::0.25, "spring"::1.5, "summer"::0.08, "autumn"::0.0];
	map<string, float> energy_map10 <- ["winter"::0.25, "spring"::1.0, "summer"::0.08, "autumn"::0.0];
	map<string, float> energy_map11<- ["winter"::0.0, "spring"::2.5, "summer"::0.5, "autumn"::0.25];
	map<string, float> energy_map12<- ["winter"::2.0, "spring"::2.0, "summer"::2.5, "autumn"::2.25];
	
	
	float get_energy (int tree_type){
		float energy;
		switch tree_type{
			match 1{
				energy <- energy_map1[current_season];
			}
			match 2{
				energy <- energy_map2[current_season];
			}
			match 3{
				energy <- energy_map3[current_season];
			}
			match 4{
				energy <- energy_map4[current_season];
			}
			match 5{
				energy <- energy_map5[current_season];
			}
			match 6{
				energy <- energy_map6[current_season];
			}
			match 7{
				energy <- energy_map7[current_season];
			}
			match 8{
				energy <- energy_map8[current_season];
			}
			match 9{
				energy <- energy_map9[current_season];
			}
			match 10{
				energy <- energy_map10[current_season];
			}
			match 11{
				energy <- energy_map11[current_season];
			}
			match 12{
				energy <- energy_map12[current_season];
			}
		}
...
species plant_seed parent: tree_part {
	...
	reflex create_tree when: (!has_tree) {
		create trunk {
			tree_type <- myself.tree_type ;
	...
	aspect default {
		...
		draw ("Tree type" + tree_type) color:#black size:displatTextSize perspective:false;		
	}
...
species burgeon parent: tree_part {
	...
	reflex growth {
		energy <- energy + first(season).get_energy(tree_type);		
	}
...
experiment L_Tri type: gui autorun: false {
