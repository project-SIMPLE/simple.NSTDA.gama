...
global{
	...
	int map_size <- 100; //map size in #m
	int sub_map_size <- 25;
	...
	int initial_n_gibbon <- 200; //num of gibbons in map
	int initial_n_baby_deer <- 200; //num of deers in map
	int initial_n_of_big_tree <- 500; //num of big-sized trees
	int initial_n_of_medium_tree <- 500; //num of medium-sized trees
	int initial_n_of_small_tree <- 500; //num of small-sized trees
...
species big_tree{
	bool on_fire <- false;
	aspect default{
		draw circle(2#m) color:on_fire? #black:rgb(24, 105, 21);
	}
...
species medium_tree{
	bool on_fire <- false;
	aspect default{
		draw circle(1#m) color:on_fire? #black:rgb(97, 135, 47);
	}
	action count_burnt_trees{
		number_of_burnt_trees <- number_of_burnt_trees + 1;
	}
...
species small_tree{
	bool on_fire <- false;
	aspect default{
		draw circle(0.5#m) color:on_fire? #black:rgb(135, 135, 47);
	}
	action count_burnt_trees{
		number_of_burnt_trees <- number_of_burnt_trees + 1;
	}
...
species inhabitant_baby_deer skills:[moving]{
	...
	aspect default{
		draw square(1#m) color: is_alerted ? rgb(255, 149, 10) : #purple;
	}
	reflex evacuate when:is_alerted{
...
autorun: true
{
    ...
    action jump_to_sub_map{
    	...
    	create simulation with:[shape::sub_shape, color::firebreak_color,
    		main_model::false, 
    ...
    output {
        display Main_map type: 3d axes: false {
			...
			event 'j' action: jump_to_sub_map;
